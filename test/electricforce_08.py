...
def boundary_condition_phi():
    #BoundaryAD
    phi[0:1, 0:n] = phi[1:2, 0:n]
    #WallAB
    phi[0:ms, 0:1] = phi[0:ms, 1:2]
    #WallCD
    phi[0:ms, n:n+1] = phi[0:ms, n-1:n]
    #BoundaryBC
    phi[ms:ms+1, 0:n+1] = phi[ms-1:ms, 0:n+1]
    #ElectrodeEF
    phi[int(E_x / deltax + 1):int(F_x / deltax + 1)+1, 0:1] = phi_electrodeEF
    #ElectrodeGH
    phi[int(G_x / deltax + 1):int(H_x / deltax + 1)+1, 0:1] = phi_electrodeGH
    if electrode_number == 4 or electrode_number == 8:
        if electrode_pattern == "line":
            #ElectrodeIJ
            i = int(I_x / deltax + 1)
...
graph02()
LineMessage()
LineFigure("electricalpotential(t=" + str(t) + ").png")
LineFigure("electrofield(t=" + str(t) + ").png")
LineFigure("electrofield(enlarge, t=" + str(t) + ").png")
t = deltaT
while t <= T:
    print "t =" + str(t)
    #電化保存則
    ...
    if t == deltaT or int(t/deltaT) % 50 == 0:
        ...
        LineFigure("electrofield(t=" + str(t) + ").png")
        LineFigure("electrofield(enlarge, t=" + str(t) + ").png")
        LineFigure("F(t=" + str(t) + ").png")
        LineFigure("F(enlarge, t=" + str(t) + ").png")
        LineFigure("velocity(t=" + str(t) + ").png")
        LineFigure("velocity(enlarge, t=" + str(t) + ").png")
